...
## 铁律，所有测试用例发现的问题，都必须修复，不能跳过或者忽略
## 铁律，修改任何代码文档后，都必须同步提交git到本地，并且每次提交只提交自己修改的部分，不要全部提交。
## 铁律，在实现业务框架的时候发现核心框架的问题，必须立即修复，不能延迟到后续版本
## 铁律，发现任何文档或者代码中的问题，必须立即修复，不能延迟到后续版本
## 铁律，实现业务逻辑的时候，必须使用TDD模式，先编写测试用例，再实现功能
## 铁律，所有新功能实现的时候都需要先查看你的记忆库内的内容。优先根据记忆库内的实现。

## 铁律，组件必须做到单一职责。
## 铁律，当一个类或者文件行数超过500行时，必须重构拆分，做到高类聚，低耦合。

## 项目内模块引入方式都使用 implementation(projects.libs.lkoCore) 这种模式 
