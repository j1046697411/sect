...
管理 Claude Code 会话历史 - 列出、加载、设置别名和编辑存储在 `~/.opencode/sessions/` 中的会话。
...
```bash
node -e "
const sm = require((process.env.CLAUDE_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.opencode'))+'/scripts/lib/session-manager');
const aa = require((process.env.CLAUDE_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.opencode'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.opencode/sessions/' + session.filename);
console.log('');
...
* 会话以 Markdown 文件形式存储在 `~/.opencode/sessions/`
* 别名存储在 `~/.opencode/session-aliases.json`
