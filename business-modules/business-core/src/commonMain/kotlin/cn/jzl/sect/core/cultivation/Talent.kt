 ...
 */
enum class AptitudeLevel(val description: String) {
    GENIUS("天才"),      // 平均值 >= 80
    EXCELLENT("优秀"),   // 平均值 >= 60
    GOOD("良好"),        // 平均值 >= 40
    AVERAGE("普通"),     // 平均值 >= 20
...
data class Talent(
     ...
     * @return 资质评级枚举
     */
    fun getAptitudeLevel(): AptitudeLevel {
        val average = getAverageAptitude()
        return when {
            average >= 80.0 -> AptitudeLevel.GENIUS
            average >= 60.0 -> AptitudeLevel.EXCELLENT
            average >= 40.0 -> AptitudeLevel.GOOD
            average >= 20.0 -> AptitudeLevel.AVERAGE
            else -> AptitudeLevel.POOR
        }
    }
}
