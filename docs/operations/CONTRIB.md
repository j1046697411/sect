...
| 模块 | 路径 | 说明 |
|------|------|------|
| client | `client/` | 应用主模块（桌面/Android/Web/WASM） |
...
| 命令 | 说明 |
|------|------|
| `./gradlew :client:run` | 运行桌面版 Demo (JVM) |
| `./gradlew :client:hotReloadJvmDev` | 热重载开发模式 |
...
| 命令 | 说明 |
|------|------|
| `./gradlew :client:lint` | 运行 lint 检查 |
| `./gradlew :client:lintFix` | 运行 lint 并自动修复 |
...
| 命令 | 说明 |
|------|------|
| `./gradlew :client:packageDmg` | 打包 macOS 应用 |
| `./gradlew :client:packageMsi` | 打包 Windows MSI |
| `./gradlew :client:packageDeb` | 打包 Linux DEB |
...
A: 尝试重新运行 `./gradlew :client:hotReloadJvmDev`，确保没有编译错误。
...
- [游戏核心玩法](../design/sect_cultivation_core_gameplay.md)
- [项目规范](../AGENTS.md)
